...
cd /d "%~dp0"
title COMPACS RAG Desktop (console)

REM ROOT = script directory (repo root or assembled Part1).
set "ROOT=%CD%"

REM Prefer fresh build output; fall back to flat dist layout (main.exe next to START.cmd).
...
echo.
echo Starting COMPACS console (direct RAG, no UI API)...
echo.
pushd "%APPDIR%"
"%APPDIR%\main.exe" --console
set "RC=%ERRORLEVEL%"
popd
exit /b %RC%
